...
public class Player extends Entity implements Battleable, OverlapAction {
    ...
    public List<String> getBuildables(boolean zombiesExist) {
        return inventory.getBuildables(zombiesExist);
    }
    ...
    public void onOverlap(GameMap map, Entity entity) {
        if (entity instanceof Enemy) {
            ...
            if (((Collectable) entity).isCollectable() && pickUp(entity)) {
                ((Collectable) entity).onPickUp(map);
            }
        }
    }
    ...
    public void buff() {
        ...
        if (effectivePotion != null) {
            ...
            for (BattleItem item : getBattleItems()) {
                playerBuff = item.applyBuff(playerBuff);
            }
        }

        battleStatistics = BattleStatistics.applyBuff(battleStatistics, playerBuff);
    }

    public void useItems(Game game) {
        for (BattleItem item : getBattleItems()) {
