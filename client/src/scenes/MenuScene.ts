  ...
  create() {
    ...
    this.generateGame = new TextButton(
      ...
      this.renderer.height / 4 +
        ...
        20 +
        125,
      window.Config.localisation.main_menu.buttons.generate_game,
      {
        fontSize: "25px",
        fontFamily: "main_menu-font",
      },
      async () => {
        const configs = await API.getConfigs();
        // const coordinates = Array.from(Array(-50, 50).keys())
        var coordinates = []
        for (var i = -50; i <= 50; i ++) {
          coordinates.push(i);
        }
        ...
        const result = await Swal.fire({
          title: window.Config.localisation.main_menu.buttons.generate_game,
          heightAuto: false,
          html: `
          ...
          <select
              ...
              >
              <datalist id="xStart">
              ${coordinates.map((x) => `<option>${x}</option>`).join("\n")}
              </datalist>
          </select>
          ...
          preConfirm: async () => {
            let inputValidator = async (values) => {
              ...
              if (values[1] === values[3] && values[0] === values[2]) {
                Swal.showValidationMessage("Start and End positions must be different");
                return false;
              }
              if (+values[1] > +values[3] || +values[0] > +values[2]) {
                Swal.showValidationMessage("End position cannot be above or left of the start position");
                return false;
              }
              if (!values || !values[4]) {
                Swal.showValidationMessage("You need to select a config file");
            ...
            if (await inputValidator(results)) {
              dungeon = await API.generate(+results[0], +results[1], +results[2], +results[3], results[4]);
              return !!dungeon;
            }
          },
        });
    ...
    this.tweens.add({
      delay: 3500,
      duration: 1000,
      targets: [this.newGame, this.loadGame, this.credits, this.exitGame, this.generateGame],
      x: 10,
      ease: "Bounce",
    });
